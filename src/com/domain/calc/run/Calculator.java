...
public class Calculator {
	...
	private static void run() {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
			do {
				...
				try {
					invokeCalculatorService(input, new Stack<Double>(), new Stack<Character>(),
							new InfixValidationServiceImpl());
	 ...
	 * @exception CalculatorOperandException, CalculatorValidatorException
	 */
	private static void invokeCalculatorService(String input, Stack<Double> values, Stack<Character> operators,
			IValidationService validationService) throws CalculatorOperandException, CalculatorValidatorException {
		ICalculationService calculatorService = ObjectCreationFactory
				.getObjectfactory(CalculatorConstants.CALCULATION_SERVICE)
				.getCalculationService(CalculatorConstants.INFIX, values, operators, validationService);
